...
public class TransparentWindow : MonoBehaviour
{
    ...
    private void SetClickThrough(bool through)
    {
        ...
        var toolwindow = PlayerPrefs.GetInt("minimize", 0) == 1 ? WS_EX_LEFT : WS_EX_TOOLWINDOW;
        Debug.Log(PlayerPrefs.GetInt("minimize", 0));

        if (through)
        {
            SetWindowLong(windowHandle, GWL_EXSTYLE, WS_EX_LAYERD | WS_EX_TRANSPARENT | toolwindow);
        }
        else
        {
            SetWindowLong(windowHandle, GWL_EXSTYLE, WS_EX_LEFT | toolwindow);
        }
    }
    ...
    void Start()
    {
        ...
        // ウィンドウモードで、ディスプレイの解像度に合わせてウィンドウのサイズを設定
